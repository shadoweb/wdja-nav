...
function get_collect_tdk(id){
    var url = get_id(id).value;
    if(url != ''){
      var domain = document.domain;
      var port = window.location.port;
      var url = '//'+domain+':'+port+'/collect.php?type=tdk&url=' + url;
      var ajax = createXMLHttpRequest();
      ajax.open('get',url);
      ajax.send(null);
      ajax.onreadystatechange = function () {
         if (ajax.readyState==4 &&ajax.status==200) {
              var rtext = JSON.parse(ajax.responseText);
              document.form.weburl.value = rtext['url'];
              document.form.webtitle.value = rtext['title'];
              document.form.webkeywords.value = rtext['keywords'];
              document.form.webdescription.value = rtext['description'];
              document.form.seotitle.value = rtext['title'];
              document.form.seokeywords.value = rtext['keywords'];
              document.form.seodescription.value = rtext['description'];
              if(rtext['icon'] != '') {
               document.form.webicon.value = '';
               document.form.webicon.value = rtext['icon'];
              }
          }
      }
    }
